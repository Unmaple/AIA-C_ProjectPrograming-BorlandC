...
ABSTRACT:
		...
		B.newmouse
VERSION: 3.0
//0224曾明杰为MR系列函数（即显存的缓存）增加两个适配函数，共计69lines
0308维修NEWMOUSE避免边界留痕
***************************/
int MouseX;
int MouseY;
int MouseS;
...
void MRmouse(int x,int y)//此处利用mimage库向缓存中写入鼠标图像
{
	...
	switch(MouseS)
	{
		case 1:                                  //手势鼠标
		{
			MRBar(x,x+13,y,y+13,0);
			MRBar(x+4,x+9,y+4,y+9,14);
		}
			break;
		case 2:                        //光标
		{
			MRLine(x+1,y-1,x+9,y-1,0);
			MRLine(x+1,y+15,x+9,y+15,0);
			MRLine(x+5,y-1,x+5,y+15,0);
		}
			break;
		case 3:                        //十字
		{

			MRLine(x-1,y+7,x+11,y+7,15);
			MRLine(x+5,y-1,x+5,y+15,15);
		}
			break;
		default:              //默认鼠标
		{

			MRLine(x,y,x,y+13,15);//white
			MRLine(x+1,y+1,x+1,y+12,15);
			MRLine(x+2,y+2,x+2,y+11,15);
			MRLine(x+3,y+3,x+3,y+10,15);
			MRLine(x+4,y+4,x+4,y+12,15);
			MRLine(x+5,y+5,x+5,y+9,15);
			MRLine(x+5,y+11,x+5,y+14,15);
			MRLine(x+6,y+6,x+6,y+9,15);
			MRLine(x+6,y+13,x+6,y+15,15);
			MRLine(x+7,y+7,x+7,y+9,15);
			MRLine(x+8,y+8,x+8,y+9,15);
			MRLine(x+9,y+9,x+9,y+9,15);
			MRLine(x-1,y-1,x-1,y+14,DARKGRAY);//忘了序号直接打枚举
			MRLine(x-1,y+14,x+3,y+11,DARKGRAY);
			MRLine(x+3,y+11,x+3,y+12,DARKGRAY);
			MRLine(x+3,y+12,x+4,y+13,DARKGRAY);
			MRLine(x+4,y+13,x+4,y+14,DARKGRAY);
			MRLine(x+4,y+14,x+7,y+17,DARKGRAY);
			MRLine(x+7,y+17,x+7,y+13,DARKGRAY);
			MRLine(x+7,y+13,x+6,y+12,DARKGRAY);
			MRLine(x+6,y+12,x+6,y+11,DARKGRAY);
			MRLine(x+6,y+11,x+5,y+10,DARKGRAY);
			MRLine(x+5,y+10,x+11,y+10,DARKGRAY);
			MRLine(x+11,y+10,x-1,y-2,DARKGRAY);
		}
		break;
	}
}
void mouse(unsigned int x,unsigned int y)
{
...
void newmouse(int *nx,int *ny,int *nbuttons)
{
	...
	*nbuttons = buttonsn;
	if(buttons0 == *nbuttons)
		*nbuttons = 0;    //使得能连续按键
	if(*nx == x0 && *ny == y0 && buttonsn == buttons0)
		return;            //鼠标状态不变则直接返回S

	clrmous(x0,y0);        //说明鼠标状态发生了改变
	save_bk_mou(*nx,*ny);
	drawmous(*nx,*ny);//20230113注：需要整合进图层动画中并作为最高图层存在，否则此处贴回背景时极易拖影（在此也可看出小物件先清除再绘制观感仍然不错，仅大物件且持续移动时闪屏严重影响观感）
}

void MRnewmouse(int *nx,int *ny,int *nbuttons)
{
	int xn,yn,buttonsn;
	...
	*nbuttons = buttonsn;
	if(buttons0 == *nbuttons)
		*nbuttons = 0;    //使得能连续按键
//本用于加速的”不动即返回“此处起反作用，已删除（可与上函数做对比）
	MRmouse(*nx,*ny);//20230113注：需要整合进图层动画中并作为最高图层存在，否则此处贴回背景时极易拖影
//（在此也可看出小物件先清除再绘制观感仍然不错，仅大物件且持续移动时闪屏严重影响观感）
//224注：此处写入缓存，故不需保存背景（在以缓存为主页面使用，在无动画页面可同时使用原版
}
...
void clrmous(int nx,int ny)//清除鼠标
{
	if(flag==1)
	{
		...
		mouse(nx,ny);
		putimage(nx-1,ny-2,buffer,COPY_PUT);//20230113注：需要整合进图层动画中并作为最高图层存在，否则此处贴回背景时极易拖影
		free(buffer);
		flag=0;
