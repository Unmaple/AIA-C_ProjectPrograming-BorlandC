...
hz.c
COPYRIGHT:  Not Us
FUNCTION:   render them, can both support normal output and MemoryRender Baseline
DATE:       2023/3/5(Add MR-series support)
Available code lines:ZMJ-4lines
******************************************/
#include <graphics.h>
#include<conio.h>
#include<stdio.h>
...
//以下为曾明杰已修改的代码，更换核心接口但没有其他方面大的改动，考虑到兼容性放置在新函数中，并不计入本人代码量
//0404清除边界检测（由MRPixel执行边界检测）但失去自动换行功能
void MRputhz(int x, int y,char *s,int flag,int part,int color)
{
	FILE *hzk_p=NULL;                                       //定义汉字库文件指针
	unsigned char quma,weima;                 //定义汉字的区码和位码
	unsigned long offset;                          //定义汉字在字库中的偏移量
		...
		case 16 :
				  {
					...
					while(*s!=NULL)
					{
						while ((*s!=NULL))//0404清除边界检测（由MRPixel执行边界检测）
						{
							y=y0;
							quma=s[0]-0xa0;                      //求出区码
							weima=s[1]-0xa0;                     //求出位码
		...
		case 48:
				  {
					...
					while(*s!=NULL)
					{
						while ((*s!=NULL))//0404清除边界检测（由MRPixel执行边界检测）但失去自动换行功能
						{
							y=y0;
							quma=s[0]-0xa0;                      //求出区码
							weima=s[1]-0xa0;                     //求出位码
